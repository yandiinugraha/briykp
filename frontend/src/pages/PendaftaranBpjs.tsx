            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-6">
                <div className="flex justify-between items-center p-4 border-b border-gray-100 bg-gray-50">
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            onClick={selectAll}
                            className="text-sm font-medium text-gray-600 hover:text-bri-blue underline decoration-dotted underline-offset-4"
                        >
                        ...
                        <thead className="bg-gray-50/50">
                            <tr>
                                <th className="px-6 py-4 text-left text-xs font-black text-gray-500 uppercase tracking-wider w-10">
                                    <input
                                        type="checkbox"
                                        className="rounded text-bri-blue focus:ring-bri-blue w-4 h-4"
                                ...
                                <th className="px-6 py-4 text-left text-xs font-black text-gray-500 uppercase tracking-wider">Tgl Mutasi</th>
                                <th className="px-6 py-4 text-left text-xs font-black text-gray-500 uppercase tracking-wider">Kelompok / Kelas</th>
                                <th className="px-6 py-4 text-left text-xs font-black text-gray-500 uppercase tracking-wider">Status BPJS</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-gray-500">
                                        <div className="flex justify-center mb-4">
                                            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-bri-blue"></div>
                                        </div>
                                ...
                                candidates.map((p) => (
                                    <tr key={p.id_peserta} className={selectedIds.includes(p.id_peserta) ? 'bg-blue-50/30' : 'hover:bg-gray-50'}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-medium">
                                            {p.tgl_mutasi ? new Date(p.tgl_mutasi).toLocaleDateString('id-ID') : '-'}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full bg-red-100 text-red-800">
                                                {p.status_bpjs?.nama || 'TIDAK TERDAFTAR'}
                                            </span>
                                        </td>
                                    </tr>
                                ))
